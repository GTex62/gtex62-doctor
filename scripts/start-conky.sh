...
export GTEX62_CONKY_SUITE_ID="doctor"
...
if [[ ! -f "$RUNTIME_ROOT/core.toml" || ! -f "$RUNTIME_ROOT/suites/doctor.toml" ]]; then
  "$SUITE_DIR/scripts/bootstrap-runtime-root.sh" "$RUNTIME_ROOT"
fi

PIDS_DIR="$CACHE_ROOT/runtime/pids"
LAUNCHER_PID_FILE="$PIDS_DIR/doctor-launcher.pid"
CONKY_PID_FILE="$PIDS_DIR/doctor-conky.pid"
...
choose_palette() {
  ...
  local cache_last="$cache_dir/doctor-palette"
  ...
  # Combo hand-off: if a palette override was passed in (e.g. by the conk launcher
  # combo launch), use it directly instead of prompting,
  ...
  if [[ -n "${GTEX62_CONKY_PALETTE_OVERRIDE:-}" ]]; then
    ...
    if [[ -n "$override_found" ]]; then
      export CONKY_DOCTOR_PALETTE="$override_found"
      echo "$CONKY_DOCTOR_PALETTE" > "$cache_last"
      echo "Doctor palette: $CONKY_DOCTOR_PALETTE (from combo selection)"
      return 0
    else
      echo "Warning: palette '$GTEX62_CONKY_PALETTE_OVERRIDE' not found for Doctor; prompting instead."
    fi
  fi
  ...
  export CONKY_DOCTOR_PALETTE="${PALETTES[$((choice - 1))]}"
  echo "$CONKY_DOCTOR_PALETTE" > "$cache_last"
}

choose_wallpaper() {
  local cache_dir="$CACHE_ROOT/runtime"
  local cache_last="$cache_dir/doctor-wallpaper"
  local default_choice=""
  local choice=""
  local last=""
  ...
  if [[ -n "${GTEX62_CONKY_WALLPAPER_OVERRIDE:-}" ]]; then
    if [[ "$GTEX62_CONKY_WALLPAPER_OVERRIDE" == "none" ]]; then
      ...
      echo "Doctor wallpaper: none (from combo selection)"
      return 0
    fi
    local override_found=""
    local w=""
    ...
    if [[ -n "$override_found" ]]; then
      ...
      echo "Doctor wallpaper: $override_found (from combo selection)"
  ...
  if [[ "${#WALLS[@]}" -eq 1 && -z "$default_choice" ]]; then
    ...
    echo "Available wallpapers for gtex62-doctor:"
    printf "0) None\n"
    for i in "${!WALLS[@]}"; do
      printf "%d) %s\n" "$((i + 1))" "${WALLS[$i]}"
    done
...
if [[ -x "$CORE_LAUNCHER" ]]; then
  if command -v setsid >/dev/null 2>&1; then
    setsid -f "$CORE_LAUNCHER" --suite doctor >/dev/null 2>&1
  else
    nohup "$CORE_LAUNCHER" --suite doctor >/dev/null 2>&1 &
    disown || true
  fi
  exit 0
fi
...
nohup conky -c "$SUITE_DIR/widgets/doctor-main.conky.conf" >/dev/null 2>&1 &
disown || true
exit 0
